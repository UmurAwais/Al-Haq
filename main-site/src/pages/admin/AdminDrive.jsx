    ...
    <div className="pb-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight uppercase">Resource Drive</h1>
            <p className="text-sm text-slate-500 font-bold uppercase tracking-widest opacity-60 flex items-center gap-2">
                <Cloud size={14} className="text-brand-accent" /> 
        ...
        <div className="p-8">
          {loading ? (
             <div className="py-32 flex flex-col items-center justify-center">
               ...
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Querying Storage Volume...</p>
             </div>
          ) : filteredFiles.length === 0 ? (
             <div className="py-32 text-center text-slate-300 border-2 border-dashed border-slate-100 rounded-[40px]">
                <FolderOpen size={64} className="mx-auto mb-4 opacity-10" />
                <h3 className="text-xl font-black uppercase tracking-tight">No Resources Found</h3>
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredFiles.map((file) => (
                  <div 
                    ...
                    className="group bg-slate-50 border border-slate-200 rounded-[30px] p-5 hover:bg-white hover:shadow-2xl hover:scale-[1.02] hover:-translate-y-1 transition-all cursor-pointer relative overflow-hidden active:scale-95 duration-300"
                  >
                     <div className="absolute top-0 left-0 w-2 h-full bg-brand rounded-full -ml-1 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                     <div className="w-14 h-14 bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center justify-center text-brand mb-4 group-hover:bg-brand group-hover:text-white transition-all">
                        <Video size={24} />
                     </div>
                     <h3 className="text-sm font-black text-slate-800 leading-tight mb-2 line-clamp-2 uppercase tracking-tight group-hover:text-brand transition-colors">
                        {file.name}
                     </h3>
      ...
      {isModalOpen && selectedFile && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 sm:p-6 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-4xl bg-white rounded-[40px] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-500 border border-slate-200 max-h-[90vh]">
             {/* Modal Header */}
             <div className="shrink-0 p-8 sm:px-10 border-b border-slate-100 flex items-center justify-between bg-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-black text-brand uppercase tracking-[0.3em]">Resource ID: {selectedFile.id}</span>
                        <div className="h-px w-8 bg-brand/30"></div>
                    </div>
                    <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tight truncate max-w-xl">{selectedFile.name}</h2>
                </div>
             ...
             <div className="flex-1 overflow-y-auto p-8 sm:p-10 custom-scrollbar">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                   ...
                   <div className="space-y-6">
                      <div className="p-8 bg-slate-900 text-white rounded-[35px] relative overflow-hidden group shadow-2xl shadow-brand/10">
                          <div className="absolute top-0 right-0 p-10 opacity-5 -mr-10 -mt-10">
                              <Play size={160} fill="white" />
                          </div>
                          <div className="relative z-10">
                             <h3 className="text-xs font-black text-brand-accent uppercase tracking-[0.3em] mb-4">Metadata Snapshot</h3>
                             {loadingDetails ? (
                                ...
                                <div className="space-y-4">
                                   <div className="flex items-center gap-3">
                                      ...
                                      <span className="text-sm font-bold opacity-80 uppercase tracking-wide truncate">{fileDetails.title || 'Untitled Video'}</span>
                                   </div>
                                   <div className="flex items-center gap-3">
                                      <Clock size={16} className="text-brand-accent" />
                                      <span className="text-sm font-bold opacity-80 uppercase tracking-wide">Duration: {fileDetails.duration || 'N/A'}s</span>
                                   </div>
                                   <div className="flex items-center gap-3">
                                      <Eye size={16} className="text-brand-accent" />
                                      <span className="text-sm font-bold opacity-80 uppercase tracking-wide">Ready for Integration</span>
                                   </div>
                                </div>
                             ) : (
                      ...
                      <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl">
                          <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Embed Link for Courses</h4>
                          <div className="flex items-center gap-3">
                             <input 
                                readOnly
                             ...
                             />
                             <button className="px-4 py-2.5 bg-brand text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90">Copy</button>
                          </div>
                      </div>
                   </div>
                   ...
                   <div className="space-y-6">
                      <div className="aspect-video bg-slate-100 rounded-[35px] border-4 border-white shadow-xl overflow-hidden relative group">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="flex items-center justify-center gap-2 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-200 transition-all border border-slate-200"
                         >
                            <ExternalLink size={14} /> Source Link
                         </a>
                         <button className="flex items-center justify-center gap-2 py-4 bg-brand/5 text-brand rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-brand/10 transition-all border border-brand/10">
                ...
                <div className="mt-10 p-6 bg-slate-50 border border-slate-100 rounded-3xl">
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">* Resource is automatically synchronized from Al-Haq's central Google Cloud Project.</p>
                </div>
             </div>
          </div>
        </div>
